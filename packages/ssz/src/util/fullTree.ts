import {bitLength, previousPowerOf2, zeroHashes} from "./hash";
...
export function getMerkleTree(values: Buffer[], padTo: number = null): Buffer[][] {
  const layerCount = bitLength(
    (padTo === null ? values.length : padTo) - 1
  );
  if (values.length === 0) {
